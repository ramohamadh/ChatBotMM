...
class FAISSVectorStore:
    ...
    def search(self, query_embedding: np.ndarray, top_k: int = 5) -> list[tuple[dict, float]]:
        ...
        # Get results
        results = []
        for _i, (idx, similarity) in enumerate(zip(indices[0], similarities[0], strict=False)):
    ...
    def load(self, directory: Path, index_name: str = "faiss_index", metadata_name: str = "metadata.json"):
        ...
        # Verify dimension. A mismatch means the index was built with a
        # different embedding model — silently adapting would make every
        # query crash later, so fail here and let callers fall back to
        # reindexing (both call sites wrap load() in try/except).
        if self.index.d != self.embedding_dimension:
            raise ValueError(
                f"Index dimension {self.index.d} does not match the embedding "
                f"model dimension {self.embedding_dimension} — the embedding "
                "model changed; rebuild with 'chatbot rebuild'"
            )
